#!/usr/bin/env bash
# Creates demo consumer users (same password as admin).
set -euo pipefail
...
create_user() {
  ...
  local ROLE="$2"
  local EXISTING
  EXISTING=$(auth_user_id "$EMAIL" "$URL" "$SERVICE_KEY" || true)
  ...
  PGPASSWORD=postgres psql -h 127.0.0.1 -p 54322 -U postgres -d postgres -q -c \
    "UPDATE profiles SET role = '$ROLE' WHERE id IN (SELECT id FROM auth.users WHERE email = '$EMAIL');"
}

create_user "buyer@realpoint.local" "buyer"
create_user "agent@realpoint.local" "agent"

echo ""
echo "Demo logins (password: $PASSWORD):"
echo "  admin@realpoint.local  — admin"
echo "  buyer@realpoint.local  — buyer"
echo "  agent@realpoint.local  — agent"
